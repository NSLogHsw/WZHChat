...
- (void)initWithTime:(NSString *)timeStr HeaderStr:(NSString *)headerStr Name:(NSString *)nameStr Guest:(NSString *)guestStr MessageWidth:(NSString *)messageWidth MessageHight:(NSString *)messageHight ChatMessageTag:(NSInteger)chatMessageTag{
    ...
    if ([timeStr isEqualToString:@""]) {
        _lab_time.frame = CGRectMake(0, 5 * ScaleY_Num, 0, 0); //此处用于剔除相同时间，不显示
    }else {
    ...
    self.messageWidth = messageWidth;    //表情文字宽度
    self.messageHight = messageHight;    //表情文字高度
    NSInteger newMessageWidth;
    NSInteger newMessageHight;
    ...
    if ([guestStr isEqual:@"1"]) {
        ...
        _conversationView.frame = CGRectMake(43 * ScaleX_Num, CGRectGetMaxY(_lab_time.frame) + 17 * ScaleY_Num, newMessageWidth + 24 * ScaleX_Num, newMessageHight);
        _conversationView.image = [UIImage stretchableImageWithImgae:@"chatBackgroundGuest"];  //适配对话框
        _btn_chatMessage.frame = CGRectMake(43 * ScaleX_Num, CGRectGetMaxY(_lab_time.frame) + 17 * ScaleY_Num, newMessageWidth + 24 * ScaleX_Num, newMessageHight);
    }else if ([guestStr isEqual:@"0"]) {
        _lab_name.text = @"";
        _lab_name.frame = CGRectMake(0, 0, 0, 0);     //主人名字不显示
...
- (void)clickChatMessageLongBtn:(UILongPressGestureRecognizer *)gestureRecognizer{  //点击事件，预留
    NSLog(@"%ld",self.tag);
